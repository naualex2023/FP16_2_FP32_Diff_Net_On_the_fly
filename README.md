...
| `pp_dit.py` | **Core (DiT)** — `PipelineParallelDiT`: splits a Transformer backbone across **N GPUs** (1-4) by cutting the `transformer_blocks` sequence into equal chunks. Auto-detects N from model size. Hook-based; numerically identical to single-GPU. |
| `pipeline_parallel_sdxl.py` | **UNet use case** — SDXL FP32 on 2 GPUs. LoRA, schedulers, Turbo. Routes through keep-alive cache. |
| `pipeline_parallel_dit.py` | **DiT use case** — PixArt/Sana/SD3/Flux FP32 on **N GPUs** (auto-detected). Estimates model size, computes required GPU count, distributes transformer blocks + T5-XXL across stages. Routes through keep-alive cache. |
...
**auto-detects** how many GPUs are needed:
```
Auto-detect: model components (FP32) ~ 58 GB total, VRAM/GPU = 22 GB -> need 3 GPU(s)
Using 3 GPU(s): ['cuda:0', 'cuda:1', 'cuda:2']
```
Transformer blocks are split equally across N stages. T5-XXL (text_encoder_3)
is placed on the last GPU with transparent relay hooks. Available in **Generate**
mode.
...
# 3. Run tests
python -m pytest test_pp_unet.py test_pp_dit.py -v
...
- `"dit"` — DiT auto N-GPU pipeline parallel
...
model path, devices, dtype, and scheduler — so different configurations get
distinct entries.
...
| DiT FP32, 3-4 GPU (auto) | model-dependent | For 30-60 GB models |
